jQuery(function($) {
	...
	function imageViewer(image) {
		...
		//If there isn't a caption, hide the span.
		if(image.caption.length === 0) {
			$('span.img-caption', viewer_div).hide();
		}
		
		//Else show it.
		else {
			$('span.img-caption', viewer_div).show();
		}
	...
	$.each(images, function(index, image) {
		...
		//If it isn't the last image, add 8 pixels of margin.
		if(index !== images.length - 1) {
			$(image_div).css('margin-right', '8px');
		}
		...
		$(image_a).on('click', function(e) {
			...
			$('.thumbnails-list a.active').removeClass('active');
		...
		//Add this image to the total width.
		totalWidth += $(image_div).outerWidth(true);
	});

	//Setup niceScroll.
	$('#gallery .thumbnails').niceScroll({cursoropacitymin:1});
	
	//Set the thumbnails list to the correct width.
	$(thumbnailsList).css('width', totalWidth + 'px');
